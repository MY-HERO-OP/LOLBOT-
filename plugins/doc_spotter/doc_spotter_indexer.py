...
def docspotter_command(update: Update, context: CallbackContext) -> None:
    ...
    if doc_spotter_plugin_enabled:
        ...
        update.message.reply_text("Doc Spotter Plugin Disabled by the Person who deployed this LOLBOT variant 💔")
...
def manage_index_channels_callback(update: Update, context: CallbackContext) -> None:
    ...
    indexed_channels = list(client["LOLBOT_Doc_Spotter"]["Indexed_Channels"].find({"user_id": user_id}))
    keyboard = []
...
def manage_listening_groups_callback(update: Update, context: CallbackContext) -> None:
    ...
    query.answer()
    user_id = query.from_user.id
    listening_groups = list(client["LOLBOT_Doc_Spotter"]["Listening_Groups"].find({"user_id": user_id}))
    keyboard = []
...
def manage_fsub_chats_callback(update: Update, context: CallbackContext) -> None:
    ...
    query.answer()
    user_id = query.from_user.id
    fsub_chats = list(client["LOLBOT_Doc_Spotter"]["Fsub_Chats"].find({"user_id": user_id}))
    keyboard = []
