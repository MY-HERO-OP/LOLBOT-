...
def is_user_member_of_fsub_chats(sender_user_id: int, chat_id: int, client, context: CallbackContext) -> bool:
    ...
    group_config = client["LOLBOT_Doc_Spotter"]["Listening_Groups"].find_one({"group_id": str(chat_id)})
    if not group_config:
        return True

    logging.info(f"🔍 Checking FSub for sender_user_id: {sender_user_id} in chat_id: {chat_id}")
    setup_user_id = group_config["user_id"]
    fsub_chats = client["LOLBOT_Doc_Spotter"]["Fsub_Chats"].find({"user_id": setup_user_id})
...
def find_user_by_group(chat_id, client):
    ...
    record = client["LOLBOT_Doc_Spotter"]["Listening_Groups"].find_one({"group_id": str(chat_id)})
