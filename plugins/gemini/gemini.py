...
def handle_gemini_command(update: Update, context: CallbackContext):
    ...
    if not gemini_plugin_enabled:
        update.message.reply_text("Gemini AI Plugin Disabled by the Person who deployed this LOLBOT variant 💔")
        return
...
def analyze4to_handler(update: Update, context: CallbackContext):
    ...
    if not gemini_image_plugin_enabled:
        update.message.reply_text("Gemini Image Analyze Plugin Disabled by the Person who deployed this LOLBOT variant 💔")
        return
