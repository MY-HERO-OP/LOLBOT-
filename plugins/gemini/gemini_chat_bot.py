...
def toggle_chatbot(update: Update, context: CallbackContext):
    if not chat_bot_plugin_enabled:
        update.message.reply_text("LOLBOT Chatbot Plugin Disabled by the Person who deployed this LOLBOT variant 💔")
        return
    ...
    if context.chat_data[chatbot_enabled_key]:
        update.message.reply_text("LOLBOT Chatbot activated. I'm here to chat with you! 🗨️")
    else:
        update.message.reply_text("LOLBOT Chatbot deactivated. I'll miss our conversations! 💔")
...
def update_thinking_message(context, chat_id, message_id):
    ...
    while not getattr(update_thinking_message, "stop", False):
        ...
        new_text = "`LOLBOT is thinking" + "." * (count % 4) + "`" 
...
def handle_chat_message(update: Update, context: CallbackContext):
    ...
    # Send initial "LOLBOT is thinking..." message in monospace font
